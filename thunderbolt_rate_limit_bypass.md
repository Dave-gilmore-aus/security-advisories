...
The server continuously responds with `200 OK` or `400 Bad Request` (depending on payload validity) across all 20+ attempts. It completely fails to trigger an expected `429 Too Many Requests` HTTP status code.

### Log Verification

Check the backend stdout logs to see the `Rate limiting skipped` warning printing sequentially for every triggered request.
